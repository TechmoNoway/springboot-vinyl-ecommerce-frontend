...
export const AuthProvider: React.FC<{
    ...
    useEffect(() => {
        const checkAuth = async () => {
            if (!token) {
                ...
                if (decodedToken.exp && decodedToken.exp > currentUnixTimestamp) {
                    const userdetailResponse = await getCurrentUser(parseInt(decodedToken.sub as string));

                    if (userdetailResponse?.data.data) {
                        const currentUserInfo = userdetailResponse.data.data;

                        localStorage.setItem('info', JSON.stringify(currentUserInfo.id));
    ...
    return (
        <AuthContext.Provider value={{ currentUser, token, login, loginWithGoogle, logout }}>
            {children}
